...
def train_model(model, task_no, num_classes, model_criterion, dataloader_train, dataloader_valid, num_epochs,
        ...
        for index, sample in enumerate(dataloader_train):
            ...
            if use_penalized_sgd:
                ...
                if task_no == 1:
                    ...
                    # todo
                    # regulation_factor = 0.001 * (task_no - 1)
                    regulation_factor = 0.01
                    if use_awl:
                        loss = automatic_weighted_loss(origin_loss, regulation)
                    else:
                        loss = origin_loss + regulation_factor * regulation
                    if index < 2 or index % 10 == 0:
                        print("origin_loss = {} regulation_factor = {} regulation = {} loss = {}"
                              .format(origin_loss, regulation_factor, regulation, loss))
    ...
    # todo
    # 是否使用梯度曲率方法
    use_curvature_gradients_method = True
