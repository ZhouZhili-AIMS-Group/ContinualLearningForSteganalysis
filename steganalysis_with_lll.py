...
def main(dataset_steganography_list, reuse_model):
    ...
    for task_num in range(1, tasks_length + 1):
        ...
        # model = model_utils.load_model_from_file(task_num)
        # device = torch.device("cuda" if use_gpu else "cpu")
        # model.to(device)
        ...
        if task_num == 1:
            epoch = 50
        elif task_num == tasks_length:
            epoch = 80
        else:
            epoch = 80
        model = mas.mas_train(model, task_num, epoch, num_freeze_layers, no_of_classes, dataloader_train,
                              dataloader_valid, actual_lr, reg_lambda=reg_lambda, use_awl=False, use_gpu=use_gpu)
...
def init_console_log():
    ...
    log_file_name = log_file_name + '-改进的曲率算法-log10()'
    # log_file_name = log_file_name + '-omege累加的权重改变'
    # log_file_name = log_file_name + '-消融实验，不使用梯度曲率算法'
    # log_file_name = log_file_name + '-消融实验，不使用peak算法'
    # log_file_name = log_file_name + '-直接设置正则项为0的对结果测试'
...
if __name__ == '__main__':
    ...
    ste_list = [
        {'dataset': DatasetEnum.BOSSBase_256, 'steganography': SteganographyEnum.WOW},
